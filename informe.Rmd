...
datos_uso_tiempo %>% 
  ...
  mutate(porcentaje_sexo = round(porcentaje_sexo, digits = 2)) %>% 
  ggplot(aes(sexo, porcentaje_sexo)) +
  geom_col(aes(width = 0.3, fill = factor(porcentaje_sexo))) +
...
datos_uso_tiempo %>% 
  ...
  mutate(porcentaje_edad = round(porcentaje_edad, digits = 2)) %>% 
  ggplot(aes(rango_etario, porcentaje_edad)) + 
  geom_col(aes(width = 0.3, fill=factor(porcentaje_edad)))+
    labs(title = "Personas según grupos etarios", 
       x = "Grupos etarios", 
       y = "%") +
  scale_fill_discrete(name = "Sexo",
...
datos_uso_tiempo %>% 
  ...
  mutate(porcentaje_ocup = round(porcentaje_ocup, digits = 2)) %>% 
  ggplot(aes(estado_ocupacional, porcentaje_ocup)) + 
  geom_col(aes(width = 0.3, fill= factor(porcentaje_ocup)))+
  labs(title = "Personas según condicion de ocupación", 
...
datos_uso_tiempo %>% 
  ...
  geom_col(aes(fill = factor(trabajo_recreacion))) +
  scale_fill_brewer(palette = "Accent", name = "Trabajo y recreación") +
...
datos_uso_tiempo %>%
  ...
  labs(title = "Correlación entre horas dedicadas al trabajo pago y a recreación
...
datos_uso_tiempo %>% 
  ...
  cols_label(sexo = "Sexo", horas_trabajo_domestico = "Trabajo doméstico no pago",
             horas_trabajo_rem = "Trabajo remunerado") %>% 
  fmt_number(columns = c(horas_trabajo_domestico, horas_trabajo_rem),
             decimals = 2, dec_mark = ",", sep_mark = ".") %>% 
  tab_style(locations = cells_column_labels(columns = everything()),
            style = list(cell_borders(sides = "bottom", weight = px(3)),
            cell_text(weight = "bold")))
...
datos_uso_tiempo %>%
  ...
  tab_style(locations = cells_column_labels(columns = everything()),
            style = list(cell_borders(sides = "bottom", weight = px(3)),
            cell_text(weight = "bold"))) ## %>% 
##  summary_rows(fns = list("mean"), groups = TRUE) No calcula correctamente el promedio, o por lo menos es distinto de los valores anteriores
```
